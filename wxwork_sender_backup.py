...
class WXWorkSender(MessageSenderInterface):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 设置pyautogui安全配置
        pyautogui.FAILSAFE = False  # 禁用鼠标角落安全机制（自动化必需）
        pyautogui.PAUSE = 0.3  # 减少操作间隔，提高效率

        # 添加自定义安全检查
        self._safety_checks = True
    ...
    def activate_application(self) -> bool:
        """激活企业微信窗口（强制显示并置顶）"""
        try:
            # 检查当前句柄是否有效，如果无效则重新查找
            if not self.main_window_hwnd or not win32gui.IsWindow(self.main_window_hwnd):
                logger.warning("企业微信窗口句柄无效，重新查找...")
                if not self._find_wxwork_windows():
                    logger.error("重新查找企业微信窗口失败")
                    return False
                logger.info(f"重新找到企业微信窗口句柄: {self.main_window_hwnd}")
    ...
    def _safety_check(self) -> bool:
        ...
        try:
            ...
            if (current_pos.x < 10 or current_pos.x > screen_width - 10 or
                current_pos.y < 10 or current_pos.y > screen_height - 10):
                logger.warning("检测到鼠标在屏幕边缘，等待2秒...")
                time.sleep(2)
                return self._safety_check()  # 递归检查
            return True
        except Exception as e:
            logger.warning(f"安全检查失败，继续执行: {e}")
            return True
